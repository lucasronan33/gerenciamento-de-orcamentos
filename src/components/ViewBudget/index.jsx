...
function getAddressLine(address = {}) {
  ...
  return [street, cityState, maskZipCode(address.zipCode || "")]
    .filter(Boolean)
    .join(" - ");
}
...
function getItemTotal(item) {
  ...
  console.log(quantity * unityPrice * (taxes / 100 + 1));
...
export function ViewBudget() {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const response = await api.get(`/budgets/${budget._id}/pdf`, {
        responseType: "blob",
      });
      const contentDisposition = response.headers["content-disposition"] || "";
      const filenameMatch = contentDisposition.match(/filename="?([^"]+)"?/);
      const filename =
        filenameMatch?.[1] || `${basic.code}_${basic.title}_${client.name}.pdf`;
      ...
      if (status === 404) {
        toast.error(
          "Nao foi possivel encontrar a rota ou o orcamento para gerar o PDF.",
        );
      } else if (status === 401) {
        toast.error("Sua sessao expirou. Entre novamente para gerar o PDF.");
      } else {
        toast.error("Nao foi possivel gerar o PDF do orcamento.");
      }
    } finally {
  ...
  const total =
    Number(totals.total) || calculatedTotal + shipping - discount + taxes;
  ...
  return (
    <div className="span-viewBudget" onClick={handleCancel}>
      <button
      ...
      <button
        ...
        disabled={isGeneratingPdf || !budget?._id}
      ...
      >
        ...
        <section className="budget-top-grid">
          ...
          <div className="budget-meta">
            ...
            <InfoLine
              ...
              value={formatDate(budget.basic?.validUntil)}
            />
            <InfoLine
              label="Status:"
              value={
        ...
        <section className="budget-box">
          ...
          <div className="budget-client-grid">
            ...
            <div>
              ...
              <InfoLine
                label="Cidade / Estado:"
                value={[clientAddress.city, clientAddress.state]
                  .filter(Boolean)
                  .join(" / ")}
              />
              <InfoLine
                label="CEP:"
                value={maskZipCode(clientAddress.zipCode || "")}
              />
            </div>
          </div>
        </section>
                ...
                {items.length > 0 ? (
                  items.map((item, index) => (
                    <tr key={item._id + index || item.code + index || index}>
                      ...
                      <td>
                        {Number(item.taxes) > 0
                          ? `${formatQuantity(item.taxes)}%`
                          : "-"}
                      </td>
                      <td>{formatCurrency(item.quantity * item.total)}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="9" className="budget-empty-row">
        ...
        <section className="budget-summary-grid">
          <div className="budget-box budget-notes">
            ...
            <span className="text-wrap">
              {"Observações gerais sobre os itens deste orçamento."}
            </span>
          </div>
          ...
          <div className="budget-totals">
            ...
            />
            <InfoLine
              label={`Frete (Tipo: ${shippingLabels[totals.shippingType] || totals.shippingType || "SF"}):`}
        ...
        <section className="budget-box budget-conditions">
          <div>
            ...
            <InfoLine
              label={
                <>
                  <WalletCards size={16} /> Forma de Pagamento:
                </>
              }
              value={paymentMethodBudget}
            />
            <InfoLine
              label={
                <>
                  <CalendarDays size={16} /> Condições de Pagamento:
                </>
              }
              value={conditions.paymentConditions}
            />
            <InfoLine
              label={
                <>
              ...
              value={conditions.shippingTime}
            />
            <InfoLine
              label={
                <>
                  <MapPin size={16} /> Tipo do Frete:
                </>
              }
              value={shippingLabels[totals.shippingType] || totals.shippingType}
            />
            <InfoLine
              label={
                <>
          ...
          <div className="budget-text-conditions">
            <div>
              <h3>
                <Info size={20} /> Observações do Orçamento:
              </h3>
              <p>
                {conditions.obsBudget ||
                  "Prazo de validade da proposta conforme informado acima. Valores sujeitos a alteração sem aviso prévio."}
              </p>
            </div>
            <div>
              <h3>
                <FileText size={20} /> Termos e Condições:
              </h3>
              <p>
                {conditions.termsConditions ||
                  "Ao aceitar este orçamento, o cliente concorda com os termos e condições aqui descritos."}
              </p>
            </div>
          </div>
        </section>
        ...
        <footer className="budget-footer">
          <strong>
            <Heart size={18} fill="currentColor" /> Obrigado pela confiança!
          </strong>
