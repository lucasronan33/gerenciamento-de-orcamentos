...
export function BudgetContentItems() {
  ...
  const subtotal = useMemo(() => {
    ...
    const value = budget.items.reduce(
      (prev, current) => prev + current.quantity * current.total,
      0,
    );
    return value;
  }, [budget.items]);
    ...
    <>
      ...
      {budget.items.length > 0 &&
        budget.items.map((item, index) => (
          <div key={item._id + index} className="box-client">
              ...
              <Form.Root>
                <Form.ContainerInput>
                  ...
                  <Form.Input
                    ...
                    value={item.quantity}
                    onChange={(e) =>
                      updateItem(item._id, "quantity", Number(e.target.value))
                    }
                  />
                </Form.ContainerInput>
                <Form.ContainerInput>
