...
export default function RegisterContent({ modalVisible, setModalVisible }) {
  ...
  const { isLoading, isRegistered, isLoggedIn } = useSelector(
    (state) => state.auth || {},
  );
  ...
  const sanitizeRegisterData = () => {
    ...
    const email =
      validator.normalizeEmail(validator.trim(formData.email)) || "";
  ...
  const validateRegisterData = (data) => {
    ...
    if (
      data.telefone &&
      !validator.isLength(data.telefone, { min: 10, max: 11 })
    ) {
    ...
    >
      ...
      >
        <div className="w-full flex flex-col gap-[1vh] py-3">
          ...
          />
          {formErrors.nome && (
            <span className="field-helper error">{formErrors.nome}</span>
          )}
        </div>
        ...
        <div className="w-full flex flex-col gap-[1vh] py-3">
          ...
          />
          {formErrors.email && (
            <span className="field-helper error">{formErrors.email}</span>
          )}
        </div>
        ...
        <div className="w-full flex flex-col gap-[1vh] py-3">
          ...
          />
          {formErrors.senha && (
            <span className="field-helper error">{formErrors.senha}</span>
          )}
        </div>
        ...
        <div className="w-full flex flex-col gap-[1vh] py-3">
          ...
          />
          {formErrors.telefone && (
            <span className="field-helper error">{formErrors.telefone}</span>
          )}
        </div>
